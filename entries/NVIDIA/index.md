...
>
> 即使在使用开放模块时，也要确保允许使用非自由(unfree)软件，因为驱动程序的用户空间部分仍然是专有的。其他专有英伟达包有`nvidia-x11`、`nvidia-settings`以及`nvidia-persistenced`。
...
Nouveau 是英伟达显卡的自由驱动程序，提供所有英伟达显卡的图形加速功能。一般不推荐使用它，因为与英伟达的官方内核模块相比，其性能明显较差，并且需要加载不自由固件才能 reclocking（动态调整时钟频率）[^7]，抑或是不支持[^2]。尽管如此，对于希望使用完全自由操作系统的人来说，Nouveau 仍然是可行的选择，因为它不包含任何专有组件，这一点与 NVIDIA 的内核模块和用户空间库不同。

[^7]: https://download.nvidia.com/XFree86/Linux-x86_64/510.39.01/README/gsp.html
